...
# VARAIABLES
model.Ar = Var(within=NonNegativeReals, bounds=(
    0, 1526), doc='Alimentacion al reformador')
Ar = model.Ar
model.Wg = Var(model.j, within=NonNegativeReals, bounds=(
    0, None), doc='gasolina por un peso W(masa)')
Wg = model.Wg
model.x = Var(model.i, model.j, within=NonNegativeReals, bounds=(
    0, None), doc='transferencia m3/d del producto intermedio i en la mezcla del producto final j ')
x = model.x
...
model.NvlMB = Constraint(expr=x['Nvl', 'G83'] + x['Nvl', 'G90'] + x['Nvl', 'G94'] <= productosIntermedios['Nvl']
                         ['Rendimiento']*model.Destil, doc='Balance de Materiales para la Nafta Virgen ligera')

model.NvpMB = Constraint(expr=x['Np', 'G83'] + x['Np', 'G90'] + x['Np', 'G94'] + Ar <= productosIntermedios['Np']
                         ['Rendimiento']*model.Destil, doc='Balance de Materiales para la Nafta Virgen Pesada')

model.Ref = Constraint(expr=x['Ref', 'G83'] + x['Ref', 'G90'] + x['Ref', 'G94'] <=
                       productosIntermedios['Ref']['Rendimiento']*Ar, doc='Balance de Materiales para el Refromado')
...
model.wgasolina = Constraint(model.j, rule=wgasolina_rule,
                             doc='El peso en masa de todos los productos intermedios tiene que ser mayor o igual que el peso en masa del producto final j')
...
model.gasolinaj = Constraint(
    model.j, rule=gasolinaj_rule, doc='Cambio de variable')


# calidad
model.calidad = ConstraintList()
for j in model.j:
    model.calidad.add(sum(x[i, j]*productosIntermedios[i]['RBN']
                      for i in model.i) - productosFinales[j]['RBNmin']*gx[j] >= 0)
    model.calidad.add(sum(x[i, j] * productosIntermedios[i]['Densidad']
                      for i in model.i) - productosFinales[j]['Densidadmin']*gx[j] >= 0)
    model.calidad.add(sum(x[i, j]*productosIntermedios[i]['RVP']
                      for i in model.i) - productosFinales[j]['RVPmax']*gx[j] <= 0)
    model.calidad.add(sum(x[i, j]*productosIntermedios[i]['PAzufre']
                      for i in model.i) - productosFinales[j]['Azufemax']*gx[j] <= 0)
...
if (resultados.solver.status == SolverStatus.ok) and (resultados.solver.termination_condition == TerminationCondition.optimal):
    ...
    for i in model.i:
        ...
        stream_results.loc[i, 'Total'] = round(
            sum(x[i, j]() for j in model.j), 1)
        if i == 'Ref':
            stream_results.loc[i, 'Available'] = round(
                productosIntermedios[i]['Rendimiento']*value(Ar), 1)
        elif i == 'Np':
            np = productosIntermedios[i]['Rendimiento']*value(model.Destil)
            stream_results.loc[i, 'Available'] = round(np-value(Ar), 1)
        else:
            stream_results.loc[i, 'Available'] = round(
                productosIntermedios[i]['Rendimiento']*value(model.Destil), 1)

    stream_results['Unused (Slack)'] = stream_results['Available'] - \
        stream_results['Total']
